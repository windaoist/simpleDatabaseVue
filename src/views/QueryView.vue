...
const editableColumns = computed(() => {
  ...
  // 优先用 columnOrder
  return columnOrder.value.length ? columnOrder.value : Object.keys(responseData.value.data.results[0]).filter(k => k !== '序号' && k !== '__colOrder');
});
...
const handleSubmit = async(row) => {
  ...
  try{
    ...
    await request.post('add-edit/edit',payload, {           headers: {
                'Accept': '*/*',
...
async function onDownload(){
  try{
    const response = await request.get('export/excel',{params:{
      keyword1: queryForm.value.keyword1,
      keyword2: queryForm.value.keyword2,
      table: queryForm.value.table
    }});
...
<el-form :inline="true" :model="queryForm">
  <div class="query-form-items">
    ...
    <el-form-item label="关键字2">
      ...
      <el-select v-model="queryForm.keyword2" clearable placeholder="次要关键字" style="width: 220px;">
    ...
    <el-form-item>
      ...
      <el-button type="success" @click="onDownload" :disabled="!responseData.data" class="download-btn">导出</el-button>
    </el-form-item>
  </div>
</el-form>
</div>
...
>
  ...
  >
    ...
    >
      ...
      <template v-else>
        <el-input v-model="row[key]" type="textarea" :autosize="true" style="width: 100%" />
      </template>
        </template>
        <template v-else>
          {{ row[key] }}
    ...
    <el-tab-pane label="相关项目" name="related-projects">
      <div class="section" v-if="responseData.data.related_data.related_projects.length > 0">
        <el-table :data="responseData.data.related_data.related_projects" border :max-height="500">
          <el-table-column 
